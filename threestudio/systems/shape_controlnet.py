...
class DreamFusion(BaseLift3DSystem):
    ...
    def configure(self):
        ...
        self.nvdiff_renderer = threestudio.find(self.cfg.mesh_fitting_renderer_type)(
    ...
    def training_step(self, batch, batch_idx):
        if self.true_global_step < 1000:
            ...
            sampler = self.mesh_renderer(**batch)
            sampler = sampler["comp_rgb"] * 255
            sampler = sampler.byte()
            #print(comp_rgb_img)
            sampler = Image.fromarray(sampler.cpu().squeeze(0).numpy())
            sampler.save('sample_renderer.png')


            guidance_out = self.guidance(
                cond_inp_resized, sampler, prompt_utils, rgb_as_latents=False, **batch
            )
